...
export function renderRunEvent(
  ...
  if (event.name === "tool_output") {
    closeOpenLine(output);
    const thread = cliState.codexThreadId
      ? `, thread ${cliState.codexThreadId}`
      : "";
    write(
      output,
      `[action completed] ${output.currentTool || "tool"}${thread}\n`,
    );
...
function renderRawEvent(data: unknown, output: RunOutputState): void {
  ...
  if (
    rawEvent.type === "response.reasoning_summary_text.delta" &&
    rawEvent.delta
  ) {
